<script lang="ts">
	...
	onMount(() => {
		const sectionElements: Element[] = [].slice.call(
			container.querySelectorAll('.scrollable-section')
		);
			...
			(entries) => {
				entries.forEach((e) => {
					...
					if (e.isIntersecting) {
						if (
							visible.length > 0 &&
							sections.indexOf(id) >= 0 &&
							sections.indexOf(id) < sections.indexOf(visible[0])
						) {
							visible.unshift(id);
						} else {
							visible.push(id);
